document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- PŁYNNA ANIMACJA TYTUŁU STRONY (PO JEDNEJ LITERCE) ---
    const titleText = "@rapldez";
    let titleIndex = 0;
    ...
    function animateTitle() {
        if (!isDeleting) {
            ...
            if (titleIndex === titleText.length) {
                setTimeout(() => { isDeleting = true; animateTitle(); }, 1500); // pauza po pełnym wpisaniu
                return;
            }
        } else {
            titleIndex--;
            document.title = titleText.substring(0, titleIndex) || "\u200B";
            if (titleIndex === 0) {
                setTimeout(() => { isDeleting = false; animateTitle(); }, 500); // pauza po wyczyszczeniu
                return;
            }
        }
        setTimeout(animateTitle, isDeleting ? 100 : 200); // stały, płynny krok po 1 literze
    }
    animateTitle();
});
